...
// ═══════════════════════════════════════════════════════════
// SLOT LEASING — active-use awareness so we never steal a slot
// that a live demo is currently showing.
// ═══════════════════════════════════════════════════════════
// Each open demo page heartbeats its slot every ~25s. A slot's lease
// (business slug + last-seen time) lives in that slot's OWN sub-account
// as a custom value `wwu_lease` (per-slot store → no cross-slot write
// races). Assignment picks the slot with the oldest/missing heartbeat
// (least-recently-active = most likely free). Heartbeats only refresh a
// lease you still own — they never overwrite someone else's, so there's
// no tug-of-war; a page that finds its slot taken re-claims a fresh one.

const LEASE_CV = 'wwu_lease';

async function getLeaseCustomValue(slot) {
  const r = await ghl('/locations/' + slot.locationId + '/customValues', slot.pit);
  if (!r.ok) return null;
  const d = await r.json().catch(() => ({}));
  return (d.customValues || []).find((c) => c.name === LEASE_CV) || null;
}

function parseLease(cv) {
  try {
    const o = JSON.parse((cv && cv.value) || 'null');
...
async function writeLease(slot, cv, obj) {
  const body = JSON.stringify({ name: LEASE_CV, value: JSON.stringify(obj) });
  if (cv && cv.id) {
    await ghl('/locations/' + slot.locationId + '/customValues/' + cv.id, slot.pit, { method: 'PUT', body });
  } else {
    await ghl('/locations/' + slot.locationId + '/customValues', slot.pit, { method: 'POST', body });
  }
}

// Read {b, s} lease for a slot (or null). b=business slug, s=lastSeen ms.
async function readLease(slot) {
  const cv = await getLeaseCustomValue(slot);
  const l = parseLease(cv);
  return { cv, lease: l };
}

// Pick the least-recently-active slot and claim it for this business.
// Falls back to round-robin if the registry can't be read.
export async function claimSlot(slug, roundRobinFallback) {
  try {
    const states = await Promise.all(
      WAKEUP_POOL.map(async (slot) => {
        const { lease } = await readLease(slot);
        const s = lease && typeof lease.s === 'number' ? lease.s : 0;
        return { slot, s };
      })
    );
    states.sort((a, b) => a.s - b.s); // oldest / never-seen first
    const chosen = states[0].slot;
    const { cv } = await readLease(chosen);
    await writeLease(chosen, cv, { b: String(slug).toLowerCase(), s: Date.now() });
    return chosen;
  } catch {
    return typeof roundRobinFallback === 'function' ? roundRobinFallback() : WAKEUP_POOL[0];
  }
}

// Heartbeat: refresh the lease ONLY if this page still owns the slot.
// Returns { owned, by }. owned=false means the slot was reclaimed by
// another demo (page should re-claim a fresh slot).
export async function heartbeatSlot(slotNum, slug) {
  const slot = WAKEUP_POOL.find((s) => String(s.slot) === String(slotNum));
  if (!slot) return { owned: false, error: 'bad slot' };
  const me = String(slug).toLowerCase();
  const { cv, lease } = await readLease(slot);
  if (!lease || String(lease.b).toLowerCase() === me) {
    await writeLease(slot, cv, { b: me, s: Date.now() });
    return { owned: true };
  }
  return { owned: false, by: lease.b };
}
